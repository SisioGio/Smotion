...
from backend.models import Picture, Album, Category
...
def new_album():
    try:
        ...
        for image in data:
            ...
            new_image = Picture(
                album_id=new_album.id, path=filename, class_name=css_class
            )
            db.session.add(new_image)
            db.session.commit()
    except Exception as e:
        print(str(e))
        return ("Error from server: " + str(e)), 400
